...
int16_t ax, ay, az, gx, gy, gz,ggy;  //MPU6050的三轴加速度和三轴陀螺仪数据
double angle;  //用于显示的角度和临时变量
int Gryo=0;
int mmo=0;
//左电机
int AIN2=13;
...
void setup() {
  ...
  attachInterrupt(0, READL, CHANGE);           //开启外部中断 
  attachPinChangeInterrupt(4, READR, CHANGE);  
}
...
void DataScope(void)
{
  ...
  data.DataScope_Get_Channel_Data(A_Z_, 3);
